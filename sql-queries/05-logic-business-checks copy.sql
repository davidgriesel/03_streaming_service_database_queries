...
-- 5.4.3 - LATE RETURNS
-- 5.4.3.1 - IDENTIFY ANY LATE RETURNS
-- 5.4.3.2 - EXPLORE LATE RETURNS
-- 5.4.3.3 - SUMMARISE LATE RETURNS

-- 5.2.1 - MISSING RETURN DATES FOLLOW-UP (183 records)
-- 5.2.1.1 - EXPLORE MISSING RETURN DATES
-- 5.2.1.2 - SUMMARISE PAYMENT STATUS FOR MISSING RETURNS

-- 5.3 - RENTAL CHECKS
-- 5.3.1 - RENTALS WITH NO PAYMENTS
-- 5.3.1.1 - IDENTIFY RENTALS WITH NO PAYMENTS
-- 5.3.1.2 - EXPLORE RENTALS WITH NO PAYMENTS

-- 5.3.2 - RENTAL WITH PAYMENTS
-- 5.3.2.1 - IDENTIFY RENTALS WITH PAYMENTS
-- 5.3.2.2 - EXPLORE RENTALS WITH PAYMENTS
-- FULL PMT | PARTIAL PMT | OVERPAYMENT (LATE RETURNS?)
...
-- payments exceeded the set rental rate.
...
-- transactions with missing return dates (Refer 5.2.1.2).
...
-- financially accounted for and contain no unresolved rentals.
-- While 135 transactions were paid in full, 24 were closed with zero-value charges,
-- and 24 payments exceeded the set rental_rate.

-- RECOMMENDATION
-- In the absence of additional business context, flag all transactions with null 
-- return dates as financially closed.
...
-- --------------------------------------------------------------------------------
-- 5.4.2 - RENTALS WITH NO PAYMENTS
-- --------------------------------------------------------------------------------
-- --------------------------------------------------------------------------------
-- 5.4.2.2 - IDENTIFY RENTALS WITH NO PAYMENTS
-- --------------------------------------------------------------------------------

-- PURPOSE
-- Identify any rentals that have no payment records.
--
    SELECT COUNT(*) AS outstanding_payments
    FROM rental r
    LEFT JOIN payment p ON r.rental_id = p.rental_id
    WHERE p.payment_id IS NULL;

-- INSIGHTS
-- There are 1452 rentals that could not be linked to payments which aligns with
-- insights uncovered during distinct value checks.

-- RECOMMENDATIONS
-- Check if payments contain split payments (Refer )

-- --------------------------------------------------------------------------------
-- 5.4.2.3 - EXPLORE RENTALS WITH NO PAYMENTS
-- --------------------------------------------------------------------------------
--
    SELECT r.*
    FROM rental r
